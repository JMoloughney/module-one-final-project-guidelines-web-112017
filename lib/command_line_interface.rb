...
class CommandLineInterface
  ...
  def get_user_name
    input = gets.chomp
    @user = User.all.find_by(name: "#{input}").name
  end

  def get_station
  	input = gets.chomp
  	@station = BikeStation.all.find_by(name: "#{input}")
  end
  ...
  def borrow_query
    ...
    input = get_station
    if !get_station == BikeStation.all.each {|station| station.name}
...
# # 	puts "Thank you for signing up #{username.name}!"
# # end
...
# 	else
# 		User.create(name: "#{input}")
# 		puts "Thank you for signing up #{User.all.last.name}"
# 	end 
# end
...
#  def borrow
#  	user = User.all.find_by(name: @user)
#  	user_rentals = Rental.all.select {|rental| user.id == rental.user_id}
#  	if user_rentals[-1].active == true
#  		puts "Sorry you an active rental, would you like to return it?"
#  		main_menu
#  	else 
#  		borrow_query
#  	end
...
#  def borrow_query
#  	puts "What location would you like to borrow from"
